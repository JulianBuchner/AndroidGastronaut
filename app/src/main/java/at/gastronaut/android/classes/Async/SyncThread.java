...
import at.gastronaut.android.classes.Sponsor;
import at.gastronaut.android.classes.TableConfig;
                                ...
                                if (obj != null && obj.getBoolean("success")) {
                                    ...
                                    if (Configuration.timestamp == 0 || Configuration.timestamp < tmp) {
                                        if (!obj.isNull("sponsors")) {
                                            ...
                                            AsyncResponse ar = output1 -> {
                                                Sponsor s = (Sponsor) output1;
                                                if (s.getBitmap() != null) {
                                                    if (Configuration.sponsorHashMap.containsKey(s.getId())) {
                                                        Bitmap tmp1 = s.getBitmap();

                                                        if (tmp1 != null) {
                                                            Configuration.sponsorHashMap.get(s.getId()).setBitmap(tmp1);
                                                        }
                                                    } else {
