...
public class PrinterCommand {
    ...
    public static byte[] POS_Print_Text(String pszString, String encoding, int codepage,
        ...
        Command.ESC_ExclamationMark[2] = (byte) (intToWidth[nWidthTimes] + intToHeight[nHeightTimes]);
        ...
        if (codepage == 0) {
            byte[] data = Other.byteArraysToBytes(new byte[][]{
                    Command.ESC_ExclamationMark, Command.ESC_t, Command.FS_and, Command.ESC_M, pbString});
                    //Command.GS_ExclamationMark, Command.ESC_t, Command.FS_and, Command.ESC_M, pbString});

            return data;
        } else {
            byte[] data = Other.byteArraysToBytes(new byte[][]{
                    Command.ESC_ExclamationMark, Command.ESC_t, Command.FS_dot, Command.ESC_M, pbString});
