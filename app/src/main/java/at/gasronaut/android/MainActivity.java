...
import at.gasronaut.android.ui.orderhistory.OrderHistoryFragment;
...
//        } else
    ...
    /*
    public void verifyOnBackgroundthread() {
        try {
            DecodeResult result = this.getChain().decode(qrCodeContent);
            final GreenCertificate data = result.getChainDecodeResult().getEudgc();
            this.runOnUiThread((Runnable)(new Runnable() {
                public final void run() {
                    MainActivity var10000 = MainActivity.this;
                    View var10001 = MainActivity.this.findViewById(1000233);
                    Intrinsics.checkNotNullExpressionValue(var10001, "findViewById(R.id.container_data)");
                    var10000.fillLayout((LinearLayout)var10001, data, result.getVerificationResult());
                }
            }));
            ...
            this.runOnUiThread((Runnable)(new Runnable() {
                public final void run() {
                    LinearLayout var10000 = (LinearLayout)MainActivity.this.findViewById(1000233);
                    TextView var1 = new TextView((Context)MainActivity.this);
                    boolean var2 = false;
                    boolean var3 = false;
                    LinearLayout var6 = var10000;
                    int var5 = false;
                    var1.setText((CharSequence)("Error on validation: " + var4.getMessage()));
                    var1.setTextColor(MainActivity.this.getResources().getColor(500194, MainActivity.this.getTheme()));
                    Unit var7 = Unit.INSTANCE;
                    var6.addView((View)var1);
                }
            }));
        }
    }*/
    ...
    public void parseMenuItem(MenuItem item) {
        ...
        if (rootMenu || Configuration.favoritesList.size() <= 0) {
            ...
            MenuItem prev = null;
            for (MenuItem tmp : Configuration.favoritesList) {
                if (prev != null) {
                    prev.nextItem = tmp;
                }
